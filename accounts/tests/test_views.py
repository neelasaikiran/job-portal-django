...
def test_initiate_password_reset_using_registered_email(client:Client, user_instance):
    ...
    response = client.post(url, request_data)
    assert response.status_code == 302
    assert Token.objects.filter(user__email=request_data["email"], token_type=TokenType.PASSWORD_RESET)
...
def test_new_password_using_valid_reset_token(client:Client, user_instance):
    url = reverse("set_new_password")
    reset_token = Token.objects.create(user=user_instance, token_type=TokenType.PASSWORD_RESET, token="abcd", created_at=datetime.now(timezone.utc))
...
def test_new_password_using_invalid_reset_token(client:Client, user_instance):
    url = reverse("set_new_password")
    reset_token = Token.objects.create(user=user_instance, token_type=TokenType.PASSWORD_RESET, token="abcd", created_at=datetime.now(timezone.utc))
